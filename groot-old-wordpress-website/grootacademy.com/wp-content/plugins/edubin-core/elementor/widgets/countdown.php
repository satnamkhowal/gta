...
class Edubin_Elementor_Widget_Countdown extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'ctw_due_date',
            [
                'label'       => esc_html__( 'Countdown Due Date', 'edubin-core' ),
                'type'        => Controls_Manager::DATE_TIME,
                'default'     => date( "Y-m-d", strtotime( "+ 1 day" ) ),
                'description' => esc_html__( 'Set the due date and time', 'edubin-core' ),
 ...
 protected function render( $instance = [] ) {
    ...
    /*
     * Convert Elementor date to WordPress timezone timestamp
     */
    $due_date = $settings['ctw_due_date'];

    if ( ! empty( $due_date ) ) {
        $timezone  = wp_timezone();
        $datetime  = new \DateTime( $due_date, $timezone );
     ...
     * Countdown parts
     */
    $render_days = '<div class="countdown-items">
        <span id="days' . esc_attr( $countdown_id ) . '" class="ctw-digits"></span>
        <span class="ctw-label ctw-label' . esc_attr( $countdown_id ) . '">' . esc_html( $settings['ctw_label_days'] ) . '</span>
    </div>';

    $render_hours = '<div class="countdown-items">
        <span id="hours' . esc_attr( $countdown_id ) . '" class="ctw-digits"></span>
        <span class="ctw-label ctw-label' . esc_attr( $countdown_id ) . '">' . esc_html( $settings['ctw_label_hours'] ) . '</span>
    </div>';

    $render_minute = '<div class="countdown-items">
        <span id="minutes' . esc_attr( $countdown_id ) . '" class="ctw-digits"></span>
        <span class="ctw-label ctw-label' . esc_attr( $countdown_id ) . '">' . esc_html( $settings['ctw_label_minuts'] ) . '</span>
    </div>';

    $render_seconds = '<div class="countdown-items">
        <span id="seconds' . esc_attr( $countdown_id ) . '" class="ctw-digits"></span>
        <span class="ctw-label ctw-label' . esc_attr( $countdown_id ) . '">' . esc_html( $settings['ctw_label_seconds'] ) . '</span>
    </div>';

    $render_label = '<div class="finished-message" style="display:none;" id="ctw-label' . esc_attr( $countdown_id ) . '">
        ' . esc_html( $end_text ) . '
    </div>';
    ...
    echo '<div ' . $this->get_render_attribute_string( 'item' ) . '>';
        echo '<div id="countdown' . esc_attr( $countdown_id ) . '">';
