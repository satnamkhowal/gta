...
$categories             = $tplMng->getDataValueArrayRequired('categories');
$categoriesWithArticles = $tplMng->getDataValueArray('categoriesWithArticles');
if (empty($categories)) : ?>
    <p>
        <?php
            printf(
...
<?php else : ?>
    ...
    <?php foreach ($categories as $category) : ?>
        <li class="dupli-help-category">
            ...
            <?php if ($category->getArticleCount() > 0 && isset($categoriesWithArticles[$category->getId()])) : ?>
                <?php $tplMng->render(
                    'parts/help/article-list',
                    [
                        'articles'   => $categoriesWithArticles[$category->getId()],
