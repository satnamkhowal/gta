...
class BootstrapRunner
{
    ...
    /**
     * Copy missing addon installer folders from the deploying site.
     *
     * Reads 'deploy_available_addons' from the overwrite params. For each addon,
     * if the folder doesn't already exist in the extracted dup-installer/addons/,
     * copies it from the source path. This allows the deploying site to enhance
     * the extracted installer with additional addon capabilities.
     *
     * @return void
     */
    protected function copyDeployAvailableAddons(): void
    {
        ...
        /** @var array<string, string> $addonsData slug => installer path */
    ...
    private function getArchiveFilePath()
    {
        if (($archiveInput = filter_input(INPUT_GET, 'archive', FILTER_SANITIZE_SPECIAL_CHARS)) != false) {
            // The parameter provides only the archive location; the file name always
            // comes from the embedded ARCHIVE_FILENAME constant, never from the request.
