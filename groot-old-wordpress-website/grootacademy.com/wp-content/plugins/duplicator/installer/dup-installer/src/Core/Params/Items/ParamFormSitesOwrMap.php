    ...
    public function isValid($value, &$validateValue = null)
    {
        ...
        try {
            foreach ($value as $item) {
                ...
                if ($item['sourceId'] == SiteOwrMap::NEW_SUBSITE_NOT_VALID) {
                    $this->setInvalidMessage(
                        'Select a source site to import. The site in this Backup cannot be imported' .
                        ' because some of its database tables were excluded when the Backup was created.'
                    );
                    return false;
                }
            ...
            $this->setInvalidMessage('The site mapping is invalid. Check the source and target site selections.');
            return false;
        }
    ...
    public static function getSourceIdsOptions()
    {
        ...
        if (is_null($sourceOpt)) {
            ...
            foreach ($archiveConfig->subsites as $subsite) {
                $label = $subsite->path;
                if (!self::isQualifiedSourceIdForImport($subsite)) {
                    $label .= ' (not importable: tables excluded from backup)';
                }
