...
class DynamicChunkRequests extends AbstractJsonSerializable
{
     ...
     * @return void
     */
    public function setDownloadUrl($url): void
    {
        $this->downloadUrl = $url;
    }
    ...
    public function request($headers = [], $data = [], $type = Requests::GET, $options = [], $resetOnFailure = true)
    {
        ...
        DupLog::trace('REQUEST HEADERS ' . SnapLog::v2str($headers));
        ...
        try {
            ...
            DupLog::infoTrace('CHUNK REQUEST FAILED: ' . $e->getMessage());
        ...
        DupLog::trace('REMOTE RESPONSE CONTENT LEN ' . SnapLog::v2str($this->lastSize));
        ...
        DupLog::trace('REMOTE RESPONSE CONTENT RANGE ' . SnapLog::v2str($contentRange) . "\n");
    ...
    {
        DupLog::trace('LAST SIZE ' . SnapLog::v2str($this->lastSize) . ' TIME ' . SnapLog::v2str($this->lastTime) . ' CHUNK TIME ' . $this->chunkTime);
        ...
        DupLog::trace('NEW SIZE ' . SnapLog::v2str($size));
     ...
     * @return void
     */
    public function setExtraData($key, $value): void
    {
