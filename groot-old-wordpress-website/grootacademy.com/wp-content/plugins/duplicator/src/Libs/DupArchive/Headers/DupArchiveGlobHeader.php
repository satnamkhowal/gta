...
class DupArchiveGlobHeader extends AbstractDupArchiveHeader
{
    ...
    /** @var string */
    protected $hash = '';
    protected \Duplicator\Libs\DupArchive\Headers\DupArchiveFileHeader $fileHeader;
     ...
     * @return void
     */
    public function setHash($content): void
    {
     ...
     */
    public function checkHash($content, $storedContent): bool
    {
     ...
     */
    public function writeToArchive($archiveHandle): int
    {
        // <G><OS>x</OS>x<SS>x</SS><HA>x</HA></G>

        $headerString = '<G><OS>' . $this->originalSize . '</OS><SS>' . $this->storedSize . '</SS><HA>' . $this->hash . '</HA></G>';

        return SnapIO::fwrite($archiveHandle, $headerString, 'Error writing glob header to archive.');
    }
    ...
    public function readFromArchive($archiveHandle, $skipGlob = false)
    {
        ...
        $this->originalSize = intval(self::getHeaderField($archiveHandle, 'OS'));
        $this->storedSize   = intval(self::getHeaderField($archiveHandle, 'SS'));
    ...
    public function readContent($archiveHandle, $validate = false)
    {
        ...
        if ($this->fileHeader->isCompressed()) {
            // Invalid data raises a PHP warning before returning false; the failure is handled by the exception.
