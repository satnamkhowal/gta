...
class MuBootstrap
{
    ...
    {
        $muAction = self::sanitizeRequestVal('dup_mu_action', '');
        if ($muAction === '') {
            return;
        }
    ...
    protected static function execInstaller()
    {
        ...
        try {
            ...
            error_log('Duplicator: mu plugin bridge security check failed');
     ...
     * @return string
     */
    public static function getBridgeHash(): string
    {
    ...
    protected static function setDefines()
    {
        ...
        define("DUPLICATOR_MU_SSDIR_NAME", 'duplicator-backups');
    ...
    protected static function sanitizeRequestVal($key, $default = '')
    {
        ...
        return filter_var($_REQUEST[$key], FILTER_SANITIZE_SPECIAL_CHARS, ['flags' => FILTER_FLAG_STRIP_LOW]);
    }
}
